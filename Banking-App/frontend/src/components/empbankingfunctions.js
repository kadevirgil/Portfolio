...
export default function EmpBankingFunctinos() {
  ...
  const [account, setAccount, accountRef] = useState({ 
    // account is the customer account you are making a transfer for
    firstName: "",
    lastName: "",
    userID: "",
    email: "",
  ...
  const [user, setUser, userRef] = useState({
    // user is the employee or admin making the internal transfer
    userID: "",
    role: "",
  });
  ...
  useEffect(() => {
    ...
    async function getAccount() {
        try {
            axios
                .get(`http://localhost:4000/account_details/${params.uID}`)
  ...
  function moveMoney() {
    ...
    if (transaction.transferType === "Deposit") {
      axios
        .put(`http://localhost:4000/deposit/${params.uID}`, transaction)
        .then((res) => {
          if (res.status !== 200) {
      ...
      axios
        .put(`http://localhost:4000/internaltransfer/${params.uID}`, transaction) 
        .then((res) => {
          if (res.status !== 200) {
            alert(`Error: ${res.statusText}`);
            return; 
      ...
      axios
        .put(`http://localhost:4000/withdraw/${params.uID}`, transaction)
        .then((res) => {
          if (res.status !== 200) {
            alert(`Error: ${res.statusText}`);
            return; 
        ...
        <Container>
          ...
          <Navbar.Collapse id="basic-navbar-nav">
            <Nav className="me-auto">
              <Nav.Link href="/home" style={{ display: userRef.current.role === "Employee" ? "none" : "block"}}>Home</Nav.Link>
              <Nav.Link href="/bankingfunctions" style={{display: userRef.current.role === "Employee" ? "none" : "block"}}>Transfers</Nav.Link>
              <NavDropdown
                title="Actions"
                id="basic-nav-dropdown"
                style={{
                  display: userRef.current.role == "Customer" ? "none" : "block",
                }}
              >
            ...
            <Nav>
              <Navbar.Text>
                Signed in as:{" "}
                <a style={{ color: "White" }}>{user.firstName}</a><br />Your ID:{" "}
                <a style={{ color: "white" }}>{user.userID}</a>
              </Navbar.Text>
              <Nav.Link
                className="mx-4"
                style={{ color: "HotPink" }}
